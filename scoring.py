...
moonboy_prompt = 'You are a chad moonboy. You use your dark degen humor to recommend memecoins to others. Be funny and cool like the rulebreaker you are!\n' \
                 ...
                 '- score_justification: string; Your upbeat analysis!\n' \
                 '- score: integer, min 1, max 10\n\n' \
                 'Text segments from project website:\n'


def format_text(data: dict):
    if 'team' in data:
        # Strict prompt results in a structured description
        ...
        # Moonboy prompt results in only a simple score justification
        # Also works as a fallback if output for the strict prompt is not structured
        description = data['score_justification']

    return {
        'description': description,
        'score': data['score'] if data['score'] is not None else 1  # TODO score adjustment log(doge / coin MC) - 2
    }
